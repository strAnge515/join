  ...
  return `
    <div class="modal-content">
      ...
      <div>
        ...
        <ul class="modal-subtask-list" id="subtaskList">${getSubtaskListHTML(task.subtasks, task.id)}</ul>
      </div>
...
/**
 * Returns an SVG img tag for the priority icon displayed in the modal.
...
/**
 * Returns HTML list items for the subtask list inside the modal.
 * @param {Array} subtasks - Array of subtask objects with title and state.
 * @param {string} taskId - The Firebase ID of the parent task.
 * @returns {string} HTML string of list items with checkboxes.
 */
function getSubtaskListHTML(subtasks, taskId) {
  if (!Array.isArray(subtasks) || subtasks.length === 0) return '<li class="modal-subtask-item">No subtasks</li>';
  return subtasks.map((subtask, index) => `
    <li class="modal-subtask-item">
      <input type="checkbox" id="subtask-${taskId}-${index}" class="modal-subtask-checkbox" data-index="${index}" ${subtask.state === true ? 'checked' : ''} />
      <label for="subtask-${taskId}-${index}">${subtask.title}</label>
    </li>
  `).join('');
}
